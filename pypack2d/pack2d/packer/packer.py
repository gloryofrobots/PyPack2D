...
def debug_bin_set(packer):
    ...
    canvas = Image.new("RGBA", (packer.bin_set.width, packer.bin_set.height), color=(128, 128, 128))
    draw = ImageDraw.Draw(canvas)

    for area in packer.areas:
        draw.rectangle([area.left, area.top, area.right - 1, area.bottom - 1], outline=choice(COLORS))

    for bin in packer.bin_set:
    ...
    def validate(self, bin):
        for binCheck in self.bin_set:
            if binCheck.is_intersect(bin) is True:
                raise BinPackerError(
                    "Validate Error! Please submit an issue. Bins are intersected : %s with %s" % (binCheck, bin))
